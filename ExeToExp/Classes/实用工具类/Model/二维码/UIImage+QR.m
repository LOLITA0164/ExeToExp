...
+(UIImage *)imageOfQRFromData:(NSData *)data codeSize:(CGFloat)codeSize{
    ...
    CIImage *originImage = [self createQRFromData:data];
//    UIImage *resultImage = [UIImage imageWithCIImage:originImage];
    //进行清晰度处理
    UIImage *resultImage = [self excludeFuzzyImageFromCIImage:originImage size:codeSize];
    return resultImage;
}
/**
 *  插入图片
...
+(UIImage *)imageOfQRFromData:(NSData *)data insertImage:(UIImage *)insertImage codeSize:(CGFloat)codeSize{
    UIImage *resultImage = [self imageOfQRFromData:data codeSize:codeSize];
    return [self imageInsertedImage:resultImage insertImage:insertImage radius:0.f];
}
/**
 *  插入图片带圆角
 */
+(UIImage*)imageOfQRFromData:(NSData *)data insertImage:(UIImage *)insertImage roundRadius: (CGFloat)roundRadius codeSize:(CGFloat)codeSize{
    UIImage *resultImage = [self imageOfQRFromData:data codeSize:codeSize];
    return [self imageInsertedImage:resultImage insertImage:insertImage radius:roundRadius];
}
/**
 *  改变颜色
 */
+(UIImage *)imageOfQRFromData:(NSData *)data red:(NSUInteger)red green:(NSUInteger)green blue:(NSUInteger)blue codeSize:(CGFloat)codeSize{
    ...
    UIImage *progressImage = [self imageOfQRFromData:data codeSize:codeSize];
    //进行渲染
...
+(UIImage *)imageOfQRFromData:(NSData *)data red:(NSUInteger)red green:(NSUInteger)green blue: (NSUInteger)blue insertImage:(UIImage *)insertImage codeSize:(CGFloat)codeSize{
    UIImage *resultImage = [self imageOfQRFromData:data red:red green:green blue:blue codeSize:codeSize];
    return [self imageInsertedImage:resultImage insertImage:insertImage radius:0.f];
}
/**
 *  改变颜色,带圆角图
 ...
 *  使用滤镜创建二维码
 */
+(CIImage*)createQRFromData:(NSData*)data{
    // 1. 实例化二维码滤镜
    CIFilter *qrFilter = [CIFilter filterWithName:@"CIQRCodeGenerator"];
    // 2. 通过KVO设置滤镜inputMessage数据
...
+ (UIImage *)imageInsertedImage: (UIImage *)originImage insertImage: (UIImage *)insertImage radius: (CGFloat)radius{
    ...
    UIImage * whiteBG = [UIImage imageWithColor:[UIColor whiteColor] withSize:CGSizeMake(insertImage.size.width*1.1, insertImage.size.height*1.1)];
